...
// The community board is now members-only (the page itself gates on this too) — enforced here
// as well so the API can't just be called directly to bypass the sign-up gate.
...
// Posting still uses a freeform, unverified display name (not the account's real identity) so
// members can stay pseudonymous — but without this, someone could post as "USCIS" or "Terra
// Support" to lend false authority to bad advice. Blocks the highest-impact impersonation targets
// rather than tying every post to the account's real name.
const RESERVED_NAME_PATTERNS = [
  /\buscis\b/i,
  /\b(dhs|ice|cbp|dol|eoir)\b/i,
  /\bdepartment of (state|homeland|labor|justice)\b/i,
  /\bterra\s+(support|team|admin|staff|official)\b/i,
  /\b(admin|administrator|moderator|official)\b/i,
];

function isReservedName(name) {
  return RESERVED_NAME_PATTERNS.some((re) => re.test(name));
}
...
async function handlePost(req, res) {
  ...
  if (name && isReservedName(name)) {
    res.status(400).json({ error: 'That name isn\'t available — it reads as an official or staff account. Please use a personal name or handle.' });
    return;
  }
