...
// See the matching function in lib/authHandlers.js — prefers a hardcoded canonical origin over
// the request's Host/X-Forwarded-Proto headers so a Stripe checkout success/cancel redirect
// can't be pointed at a non-canonical hostname. Falls back to the header-derived value only if
// SITE_URL isn't set yet.
...
async function handleCreateCheckout(req, res) {
  ...
  if (!priceId) {
    console.error(`billing-checkout: missing ${config.priceEnvVar}`);
    res.status(500).json({ error: 'Checkout is not available right now' });
    return;
  }
