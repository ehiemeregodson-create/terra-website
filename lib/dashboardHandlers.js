...
async function handleSummary(req, res) {
  ...
  try {
    ...
    //
    // Tiered so the widget always shows real alerts instead of an empty state: alerts are only
    // ever sourced from official U.S. agencies (USCIS/DHS), so they all carry country_to
    // 'United States'. A case matching that destination sees alerts relevant to it — either
    // matching its own category, or U.S.-wide alerts (category: null) — most recent 2 first.
    // A case with no U.S.-bound destination sees none; policyAlertsCoverage tells the frontend
    // to show an honest "not covered yet" note instead of fabricating content for a country
    // we haven't verified.
    ...
    const relevant = countryMatched.filter((a) => !a.category || userCategories.has(a.category));
    const policyAlerts = relevant
