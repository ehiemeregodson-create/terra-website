...
async function handleSummary(req, res) {
  ...
  try {
    ...
    //
    // Tiered so the widget always shows real alerts instead of an empty state, and stays specific
    // to *this* user's own case rather than a one-size-fits-all feed: alerts matching this user's
    // case category (e.g. "Study (student visa)") always outrank generic U.S.-wide alerts
    // (category: null), even if the generic ones are more recent — a family-based applicant
    // should see family-specific news before a general filing-rule update. Generic alerts only
    // fill remaining slots up to 2 total. Alerts are only ever sourced from official U.S.
    // agencies (USCIS/DHS), so they all carry country_to 'United States'; a case with no
    // U.S.-bound destination sees none, and policyAlertsCoverage tells the frontend to show an
    // honest "not covered yet" note instead of fabricating content for a country we haven't
    // verified.
    const byRecency = (a, b) => new Date(b.published_at) - new Date(a.published_at);
    ...
    const countryMatched = allAlerts.filter((a) => !a.country_to || userCountries.has(a.country_to));
    const categorySpecific = countryMatched.filter((a) => a.category && userCategories.has(a.category)).sort(byRecency);
    const generic = countryMatched.filter((a) => !a.category).sort(byRecency);
    const policyAlerts = [...categorySpecific, ...generic].slice(0, 2);
    const policyAlertsCoverage = countryMatched.length > 0;
