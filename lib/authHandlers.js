...
// Confirms the upload's actual bytes match its claimed Content-Type, rather than trusting the
// client's declaration outright — a client could otherwise label arbitrary bytes as "image/png"
// and have them stored and served back with that Content-Type. Checks each format's magic
// number/signature bytes.
function matchesImageSignature(buffer, contentType) {
  ...
  if (contentType === 'image/png') {
    ...
    return png.every((byte, i) => buffer[i] === byte);
  }
  ...
  try {
    if (typeof body.presetId === 'string' && AVATAR_PRESET_PATTERN.test(body.presetId)) {
      ...
      if (!matchesImageSignature(buffer, contentType)) {
        res.status(400).json({ error: 'That file doesn\'t look like a valid JPEG, PNG, or WEBP image.' });
        return;
      }
