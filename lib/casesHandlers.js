...
// POST /api/cases/log-update — the quick way for a user to say "something changed" without
// resubmitting the full intake form: an optional new stage and/or a short free-text note, both
// logged to case_events. Shared by the dashboard card's Update button and the case-edit page.
async function handleLogUpdate(req, res) {
  if (req.method !== 'POST') {
    res.status(405).json({ error: 'Method not allowed' });
    return;
  }
  ...
  const allowed = await checkRateLimit(`cases-log-update:${clientIp(req)}`, { limit: 30, windowSeconds: 3600 });
  if (!allowed) {
    res.status(429).json({ error: 'Too many requests. Please try again later.' });
    return;
  }
  ...
  const id = cleanString((req.body || {}).id, 100);
  const stage = cleanString((req.body || {}).stage, 200);
  ...
  if (!stage && !note) {
    res.status(400).json({ error: 'Add a status or a note to log an update' });
    return;
  }

  try {
    const existingRes = await supabaseRequest(`cases?id=eq.${encodeURIComponent(id)}&user_id=eq.${user.id}&select=stage`);
    ...
    if (stage && stage !== existing.stage) {
      const patchRes = await supabaseRequest(`cases?id=eq.${encodeURIComponent(id)}&user_id=eq.${user.id}`, {
        method: 'PATCH',
        body: { stage, updated_at: new Date().toISOString() },
        extraHeaders: { prefer: 'return=minimal' },
      });
      if (!patchRes.ok) {
        const detail = await patchRes.text();
        console.error('cases/log-update: stage patch failed', patchRes.status, detail);
        res.status(502).json({ error: 'Failed to update case' });
        return;
      }
      await supabaseRequest('case_events', {
        ...
        body: {
          ...
          description: `Previously: ${existing.stage}`,
        },
        extraHeaders: { prefer: 'return=minimal' },
      });
    }
    ...
    console.error('cases/log-update: request failed', err);
    await captureError(err, { route: 'cases/log-update' });
    res.status(500).json({ error: 'Failed to reach database' });
  }
}
...
module.exports = { handleList, handleCreate, handleUpdate, handleLogUpdate, handleDelete };
