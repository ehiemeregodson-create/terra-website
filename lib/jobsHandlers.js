...
// ranked by fit. No login required — this is a public tool, same as the rest of jobs.html.
  ...
  try {
    ...
    const scored = rows.map((r) => {
      ...
      if (jobType) {
        ...
        score += keywords.filter((k) => haystack.includes(k)).length * 2;
      }
      const mid = r.wage_to ? (Number(r.wage_from) + Number(r.wage_to)) / 2 : Number(r.wage_from);
    ...
    const results = scored.slice(0, 8).map(({ _score, ...r }) => r);

    res.status(200).json({ success: true, results, industries: JOBS_INDUSTRIES });
  } catch (err) {
